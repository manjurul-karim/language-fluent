...
const AllUser = () => {
  ...
  const handleMakeAdmin = (user) => {
    console.log(user);
    fetch(
      `https://a12-languagefluent-server.vercel.app/users/admin/${user._id}`,
      {
        method: "PATCH",
      }
    )
  ...
  const handleMakeInstructor = (user) => {
    console.log(user);
    fetch(
      `https://a12-languagefluent-server.vercel.app/users/instructor/${user._id}`,
      {
        method: "PATCH",
      }
    )
  ...
  const handleDelete = (user) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(
          `https://a12-languagefluent-server.vercel.app/users/${user._id}`,
          {
            method: "DELETE",
          }
        )
